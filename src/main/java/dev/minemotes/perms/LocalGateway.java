...
final class LocalGateway {
  ...
  static boolean check(ServerPlayerEntity player, String node, int opLevelFallback) {
    // 1) LuckPerms
    if (HAS_LP) {
      try {
        ...
        if (user != null) {
          ...
          if (result != null) return result.asBoolean();
        }
  ...
  private static boolean classPresent(String fqcn) {
    try { Class.forName(fqcn, false, LocalGateway.class.getClassLoader()); return true; }
